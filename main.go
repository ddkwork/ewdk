...
function(um_dp86 _target)
    ...
    list(APPEND _OPT /utf-8 /EHsc /std:c++latest)

    set(_all_objs)
    if(WDK_PCH)
        message(STATUS "PCH ignored: unity build active for ${_target}")
    endif()

    # ---- Unity Build: single compilation unit instead of per-file compile ----
    set(_unity_file "${CMAKE_CURRENT_BINARY_DIR}/_unity_${_target}.cpp")
    file(WRITE ${_unity_file} "// Unity build - auto-generated by ewdk\n")
    set(_has_cpp_sources FALSE)
    foreach(_src ${WDK_SOURCES})
        ...
        if(NOT _ext STREQUAL ".rc")
            ...
            set(_has_cpp_sources TRUE)
        endif()
    endforeach()
    if(_has_cpp_sources)
        set(_unity_obj "${_obj_dir}/${_target}_unity.obj")
        add_custom_command(
            OUTPUT "${_unity_obj}"
            COMMAND "${X86_CL}" /utf-8 /nologo /c "${_unity_file}" /Fo"${_unity_obj}" ${_inc_flags} ${_def_flags} ${_CRT} ${_OPT}
            DEPENDS "${_unity_file}"
            COMMENT "Compiling x86 unity: ${_target}"
        )
        list(APPEND _all_objs "${_unity_obj}")
    endif()
...
function(um_dll_x86 _target)
    ...
    if(CMAKE_C_COMPILER STREQUAL X86_CL)
        ...
        add_custom_command(
            OUTPUT "${_unity_obj}"
            COMMAND "${X86_CL}" /utf-8 /nologo /c "${_unity_file}" /Fo"${_unity_obj}" ${_inc_flags} ${_def_flags} ${_CRT} ${_OPT} /EHsc
            DEPENDS "${_unity_file}"
            COMMENT "Compiling x86 unity: ${_target}"
        )
        list(APPEND _all_objs "${_unity_obj}")
...
function(um_exe_mfc_x86 _target)
    ...
    add_custom_command(
        OUTPUT "${_unity_obj}"
        COMMAND "${_cc}" /utf-8 /nologo /c "${_unity_file}" /Fo"${_unity_obj}" ${_inc_flags} ${_def_flags} ${_CRT} ${_OPT}
        DEPENDS "${_unity_file}"
        COMMENT "Compiling x86 unity: ${_target}"
    )
    list(APPEND _all_objs "${_unity_obj}")
