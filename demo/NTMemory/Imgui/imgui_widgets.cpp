...
bool ImGui::TabButtonEx(const char* label, const char* icon, const ImVec2& size_arg, ImGuiButtonFlags flags, bool selected, bool bugfix)
{
    ...
    if (selected)
    {
        ...
        PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(120, 121, 241)));
        //label
        if (bugfix)
...
bool ImGui::Keybind(const char* label, const char* description, int* key, bool widget_label) {
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    //circle
    if (*v)
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.0f, ImColor(120, 121, 241), 30);
    else
...
bool ImGui::CheckboxDesc(const char* label, const char* description, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map <ImGuiID, checkboxdesc> animate;
    auto it_anim = animate.find(id);
    if (it_anim == animate.end())
    {
        animate.insert({ id, { 0.0f } });
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 arrow_size = icons->CalcTextSizeA(16.0f, FLT_MAX, 0.0f, "J");
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(36, 36, 36), 3.0f);
    ...
    RenderTextClipped(clickable.Min + ImVec2(10, 7), clickable.Max - ImVec2(24, -47), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));

    PushFont(icons); {
        window->DrawList->AddText(ImVec2(clickable.Max.x - 25, (clickable.Min.y + clickable.Max.y) / 2 - arrow_size.y / 2 + 2), ImColor(1.0f, 1.0f, 1.0f, 1.0f), "J");
    } PopFont();
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(8, 12)); // Horizontally align ourselves with the framed text
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    //active slider
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(120, 121, 241), 5.0f);

    //slider grab
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 7, grab_bb.Min.y + 1), 6.0f, ImColor(120, 121, 241), 30);
