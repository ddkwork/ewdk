...
void QObject_killTimerWithId(QObject* self, Qt::TimerId id) {
	self->killTimer(id);
}
...
int QObject_startTimer2(QObject* self, int interval, Qt::TimerType timerType) {
	return self->startTimer(static_cast<int>(interval), timerType);
}

QMetaObject__Connection* QObject_connect3(QObject* sender, QMetaMethod* signal, QObject* receiver, QMetaMethod* method, Qt::ConnectionType type) {
	return new QMetaObject::Connection(QObject::connect(sender, *signal, receiver, *method, type));
}

QMetaObject__Connection* QObject_connect4(const QObject* self, QObject* sender, const char* signal, const char* member, Qt::ConnectionType type) {
	return new QMetaObject::Connection(self->connect(sender, signal, member, type));
}
