...
Qt::TimerId QTimer_id(const QTimer* self) {
	return self->id();
}
...
void QTimer_setTimerType(QTimer* self, Qt::TimerType atype) {
	self->setTimerType(atype);
}

Qt::TimerType QTimer_timerType(const QTimer* self) {
